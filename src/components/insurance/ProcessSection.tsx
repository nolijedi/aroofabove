        ...
        <div className="space-y-6">
          {steps.map((step, index) => (
            <motion.div
              ...
              className={`bg-gradient-to-br ${step.gradient} p-6 rounded-xl shadow-md hover:shadow-xl transition-all duration-300 group relative overflow-hidden`}
            >
              {/* Background pattern */}
              ...
              <div className="relative flex items-start gap-6">
                <div className="flex-shrink-0">
                  <div className="p-4 bg-white/80 rounded-full shadow-inner group-hover:scale-110 transition-transform duration-300">
                    <step.icon className="w-8 h-8 text-roofing-orange" />
                  </div>
                </div>

                <div className="space-y-2">
                  <div className="flex items-center gap-3">
                    <span className="flex items-center justify-center w-8 h-8 bg-roofing-orange text-white rounded-full font-bold">
                      {index + 1}
                    </span>
                    <h3 className="text-xl font-semibold text-roofing-charcoal group-hover:text-roofing-orange transition-colors duration-300">
                      {step.title}
                    </h3>
                  </div>
              ...
              {index < steps.length - 1 && (
                <motion.div
                  initial={{ height: 0 }}
                  animate={{ height: "2rem" }}
                  className="absolute left-11 bottom-0 w-0.5 bg-gradient-to-b from-roofing-orange to-transparent"
